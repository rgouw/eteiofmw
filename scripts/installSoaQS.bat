...
set FMW_HOME=C:\oracle\JDeveloper\12210_SOAQS
if exist "%FMW_HOME%" goto :SOAQS_HOME_EXISTS
echo Install %FMW_HOME% 
rem echo "Current dir:" %~dp0
echo "Silent install SOA QuickStart, using response file:" %cd%\soaqs1221_silentInstall.rsp 
"%JAVA_HOME%\bin\java.exe" -jar fmw_12.2.1.0.0_soa_quickstart.jar -silent -responseFile %cd%\soaqs1221_silentInstall.rsp -nowait
:SOAQS_HOME_EXISTS
