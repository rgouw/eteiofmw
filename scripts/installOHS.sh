...
export OHS_INSTALL_JAR=fmw_12.2.1.1.0_ohs_linux64.bin
...
if [[ -d "$FMW_HOME" && ! -d "$OHS_PROD_DIR/bin" ]]; then
  #
  #Unzip hs
  if [ ! -f "$OHS_INSTALL_HOME/$OHS_INSTALL_JAR" ]; then
    if [ -f "$OHS_ZIP_HOME/$OHS_INSTALL_ZIP" ]; then
      echo Unzip $OHS_ZIP_HOME/$OHS_INSTALL_ZIP to $OHS_INSTALL_HOME/$OHS_INSTALL_JAR
      unzip -o $OHS_ZIP_HOME/$OHS_INSTALL_ZIP -d $OHS_INSTALL_HOME
    else
      echo $OHS_INSTALL_HOME/$OHS_INSTALL_ZIP does not exist!
    fi  
  else
    echo $OHS_INSTALL_JAR already unzipped
  fi
  if [ -f "$OHS_INSTALL_HOME/$OHS_INSTALL_JAR" ]; then
    ...
    $OHS_INSTALL_HOME/$OHS_INSTALL_JAR -silent -responseFile $PWD/$OHS_INSTALL_RSP
  else
    echo $OHS_INSTALL_JAR not available!./
  fi
else
  if [ ! -d "$FMW_HOME" ]; then
echo 7
